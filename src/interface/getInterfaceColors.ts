import { TerraSemantics } from "../types/Terra";
import { VscInterfaceColors } from "../types/VisualStudioCode";
import { transparentize } from "../lib/transparentize";
import { HexColor } from "../types/Color";

/**
 * Returns colors for the interface, if fed with semantic color variables.
 * With the second optional parameter, overrides can be used.
 * @param semantics TerraSemantics
 * @param overrides VscInterfaceColors
 * @returns VscInterfaceColors
 */
export const getInterfaceColors = (
  semantics: TerraSemantics,
  overrides?: VscInterfaceColors
): VscInterfaceColors => {
  const { transparent, shadow, fg, bg, border, ansi, vcs } = semantics;
  ...
  const defaultInterfaceColors: VscInterfaceColors = {
    ...
    "menu.selectionBackground": fg.light,
